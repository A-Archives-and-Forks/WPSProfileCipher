...
Release 构建使用 `windows-x64-release` preset。Windows 目标与 Crypto++ 均使用静态运行库：
...
在对应终端设置 `VCPKG_ROOT` 后运行相应 preset：
...
仓库内的 VSCode 配置包含 Windows x64、Linux x64/arm64 和 macOS x64/arm64。先在 CMake Tools 中选择并配置当前平台的 Release preset，再通过 “C/C++: Select a Configuration” 选择同名配置；对应的 `compile_commands.json` 会用于 IntelliSense。若编辑器仍保留旧诊断，执行 “C/C++: Reset IntelliSense Database”。
